...
void initSquareClipper(Clipper* clipper) {
	...
	clipper->clipperVertex[1].x = 1000;
	clipper->clipperVertex[1].y = 0;
	clipper->clipperVertex[2].x = 1000;
	clipper->clipperVertex[2].y = 1000;
	clipper->clipperVertex[3].x = 0;
	clipper->clipperVertex[3].y = 1000;
}
...
int countVertexInsideClip (Polygon* polygon, Vertex clip1, Vertex clip2) {
	...
	for (int j = 0; j < vertexCount; j++) {
		...
		firstPointPosition = (clip2.x - clip1.x) * (endPoint1.y - clip1.y) - (clip2.y - clip1.y) * (endPoint1.x - clip1.x);
		secondPointPosition = (clip2.x - clip1.x) * (endPoint2.y - clip1.y) - (clip2.y - clip1.y) * (endPoint2.x - clip1.x);
...
int countPolygonInsideClip (Shape* shapes, Vertex clip1, Vertex clip2) {
	...
	for (int i = 0; i < shapes->polygonCount; i++) {
		int counterVertexInsideClip = countVertexInsideClip(&(shapes->polygons[i]), clip1, clip2);
...
int countShapeInsideClip (VectorImage* image, Vertex clip1, Vertex clip2) {
	...
	for (int i = 0; i < image->n_component; i++) {
		int counterPolygonInsideClip = countPolygonInsideClip(&(image->shape[i]), clip1, clip2);
...
void clipVectorImageHelper(VectorImage *image, Vertex clip1, Vertex clip2) {
	...
	newVectorImage.shape = (Shape *) malloc(newVectorImage.n_component * sizeof(Shape));
	newVectorImage.color = (Color *) malloc(newVectorImage.n_component * sizeof(Color));
		...
		if (newPolygonSize == 0) {
			...
			for (int i = 0; i < polygonCount; i++) {
				...
				if (newVertexSize == 0) {
					...
					int vertexCount = image->shape[shapeIterator].polygons[i].vertexCount;
					Vertex* endPoints = image->shape[shapeIterator].polygons[i].vertices;
					...
					for (int j = 0; j < vertexCount; j++) {
						...
						if (firstPointPosition < 0 && secondPointPosition < 0) {
							newPolygon.vertices[newVertexIterator].x = endPoint2.x;
							newPolygon.vertices[newVertexIterator].y = endPoint2.y;
							newVertexIterator++;
						} else if (firstPointPosition >= 0 && secondPointPosition < 0) {
							newPolygon.vertices[newVertexIterator] = findIntersect(clip1, clip2, endPoint1, endPoint2);
							newVertexIterator++;
					...
					clonePolygon(&newPolygon, &newVectorImage.shape[newShapeIterator].polygons[newPolygonIterator]);
					newPolygonIterator++;
				}
			}
